...
function Artikel() {
  ...
  const renderPaginationButtons = (totalCards) => {
      ...
      buttons.push(
        <button
          ...
          className={`
                ...
                ${currentPage === i ? "active-page" : "bg-white border border-dark text-dark hover:bg-gray-200"}
              `}
    ...
    return <div className="flex justify-center mt-5 mb-5 space-x-2">{buttons}</div>;
  };
    ...
    <>
      ...
      <div className="container font-plus-jakarta">
        {displayedCards.map((card) => (
          <div key={card.judul_artikel} className="mb-4 flex flex-row border rounded-lg shadow-lg bg-white overflow-hidden w-3/4 mx-auto">
            <div className="flex-none w-1/4">
              <img src={`${API_GAMBAR_URL}${card.gambar_news}`} alt={card.title} className="object-cover h-full w-full" />
            </div>

            <div className="flex flex-1 p-3 flex-col justify-between">
              <h1 className="card-title break-words mb-2 text-3xl">{card.title}</h1>
              ...
              <p className="card-text mb-3" dangerouslySetInnerHTML={sliceDescription(card.deskripsi, 2)}></p>
              <a
                href={`/news/${card.id}`}
        ...
        <div className="d-flex justify-content-center mt-3 mb-5">{renderPaginationButtons(semuaCards.length)}</div>
      </div>
    </>
  );
}
