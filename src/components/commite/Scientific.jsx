...
const Scientific = () => {
  ...
  useEffect(() => {
    axios
      .get(`${API_BASE_URL}scientific`)
      .then((response) => {
        setScientificBoard(response.data);
      })
      .catch((error) => {
          ...
          <table className="table-fixed w-3/4 font-semibold" style={{ paddingLeft: "20px", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {scientificBoard.map((advisory, index) => (
                <tr key={index}>
                  <td className="border-b-2 text-center border-gray-600 px-4 py-2">{advisory.nama}</td>
                  <td className="border-b-2 text-center border-gray-600 px-4 py-2">{advisory.instansi}</td>
                </tr>
              ))}
            </tbody>
          </table>
